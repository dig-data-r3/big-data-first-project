...
from historical_stock_prices d
join (select ticker as max_ticker, max(price_date) as max_price_date FROM historical_stock_prices group by ticker) max_table on (d.ticker = max_table.max_ticker and d.price_date >= max_table.max_price_date)
) max_table on d.ticker = max_table.max_ticker
join (
select d.ticker as min_ticker, d.close_price as min_close_price
from historical_stock_prices d
join (select ticker as min_ticker, min(price_date) as min_price_date FROM historical_stock_prices group by ticker) min_table on (d.ticker = min_table.min_ticker and d.price_date <= min_table.min_price_date)
